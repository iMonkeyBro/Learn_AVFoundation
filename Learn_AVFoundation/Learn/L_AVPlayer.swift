...
class L_AVPlayer: BaseViewController {
    ...
    private func setUI() {
        ...
        playerBtn.frame = CGRect(x: 10, y: view.bounds.height-150, width: 50, height: 20)
        ...
        pauseBtn.frame = CGRect(x: 70, y: view.bounds.height-150, width: 50, height: 20)
        pauseBtn.addTarget(self, action: #selector(pauseAction), for: .touchUpInside)
        view.addSubview(pauseBtn)
        timeSlider = UISlider(frame: CGRect(x: 10, y: view.bounds.height-75, width: 222, height: 22))
